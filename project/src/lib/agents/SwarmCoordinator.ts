...
export class SwarmCoordinator {
  ...
  private hasConsensus(taskId: string): boolean {
    ...
    const agreements = results.reduce((acc, curr) => {
      const matching = results.filter(r => 
        JSON.stringify(r.result) === JSON.stringify(curr.result)
      ).length;
    ...
    return Array.from(agreements.values())
      .some(count => count / results.length >= this.config.consensusThreshold);
  }
  ...
  private getFinalResult(taskId: string): TaskResult {
    ...
    const [mostCommonResult] = Array.from(resultCounts.entries())
