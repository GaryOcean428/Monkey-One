...
export class ConversationManager extends EventEmitter {
  ...
  public async routeMessage(message: Message): Promise<Response> {
    ...
    try {
      const response = await this.withTimeout(agent.processMessage(message), this.timeout)
  ...
  private selectAgent(message: Message): BaseAgent | undefined {
    ...
    return this.getAllAgents().find(agent => this.canAgentHandleMessage(agent, message))
  }
  ...
  private canAgentHandleMessage(agent: BaseAgent, message: Message): boolean {
    ...
    return agent
      .getCapabilities()
      .some(cap => message.content.toLowerCase().includes(cap.toLowerCase()))
  }
