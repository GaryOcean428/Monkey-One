...
describe('ToolResultHandling', () => {
  describe('basic validation', () => {
    ...
    it('should reject non-number results', async () => {
      ...
      await expect(validatedTool.execute({})).rejects.toThrow(ToolExecutionError)
    })
  ...
  describe('array validation', () => {
    ...
    it('should reject invalid array elements', async () => {
      ...
      await expect(validatedTool.execute({})).rejects.toThrow('Array must contain only numbers')
    })
  })
  ...
  describe('async transformation', () => {
    it('should transform results asynchronously', async () => {
      const tool = createMockTool(42)
      const transformedTool = ToolResultHandling.withAsyncTransformation(tool, async num =>
        String(num)
      )
    ...
    it('should handle transformation errors', async () => {
      const tool = createMockTool(42)
      const transformedTool = ToolResultHandling.withAsyncTransformation(tool, async () => {
        throw new Error('Transform error')
      })

      await expect(transformedTool.execute({})).rejects.toThrow('Transform error')
    })
  })
  ...
  describe('error mapping', () => {
    it('should map known errors', async () => {
      ...
      await expect(mappedTool.execute({})).rejects.toThrow('Please try again later')
    })
    ...
    it('should pass through unknown errors', async () => {
      ...
      await expect(mappedTool.execute({})).rejects.toThrow('UNKNOWN_ERROR')
    })
    ...
    it('should handle regex patterns in error mapping', async () => {
      ...
      await expect(mappedTool.execute({})).rejects.toThrow('An error occurred')
    })
  })
  ...
  describe('composition', () => {
    it('should maintain error context through the chain', async () => {
      ...
      await expect(composedTool.execute({})).rejects.toThrow('Array must contain only numbers')
    })
  })
})
