    ...
    (set, get) => ({
      ...
      getFilteredThoughts: () => {
        ...
        return thoughts.filter(thought => {
          ...
          if (filters.collaborationId && thought.collaborationId !== filters.collaborationId) return false;
          ...
          if (filters.minImportance && (!thought.importance || thought.importance < filters.minImportance)) return false;
          if (filters.minConfidence && (!thought.confidence || thought.confidence < filters.minConfidence)) return false;
          if (filters.searchQuery && !thought.message.toLowerCase().includes(filters.searchQuery.toLowerCase())) return false;
          return true;
        });
      },
