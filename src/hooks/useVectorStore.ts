...
export function useVectorStore() {
  ...
  const findSimilarInsights = useCallback(
    async (
      embedding: number[],
      options?: {
        type?: string
  ...
  const findSimilarPatterns = useCallback(async (embedding: number[], limit?: number) => {
    setIsLoading(true)
    setError(null)
  ...
  const storeInsight = useCallback(async (insight: CodeInsight, embedding: number[]) => {
    setIsLoading(true)
    setError(null)
  ...
  const storeMetrics = useCallback(async (metrics: LearningMetric[], embedding: number[]) => {
    setIsLoading(true)
    setError(null)
