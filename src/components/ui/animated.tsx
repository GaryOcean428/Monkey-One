...
export const AnimatedElement = React.forwardRef<HTMLDivElement, AnimatedElementProps>(
  (
    ...
    const { shouldAnimate: directShouldAnimate, animationClass: directAnimationClass } =
...
export const StaggeredContainer = React.forwardRef<HTMLDivElement, StaggeredContainerProps>(
  (
    { children, className, staggerDelay = 100, animation = 'fade-in', lazy = true, ...props },
    ref
  ) => {
    ...
    return (
      <div ref={ref} className={className} {...props}>
        {childArray.map((child, index) => (
          <AnimatedElement key={index} animation={animation} delay={delays[index]} lazy={lazy}>
            {child}
          </AnimatedElement>
        ))}
      </div>
...
export const FadeIn = React.forwardRef<HTMLDivElement, FadeInProps>(
  ({ children, delay = 0, lazy = true, ...props }, ref) => (
    <AnimatedElement ref={ref} animation="fade-in" delay={delay} lazy={lazy} {...props}>
      {children}
    </AnimatedElement>
  )
)
...
export const SlideIn = React.forwardRef<HTMLDivElement, SlideInProps>(
  ({ children, direction = 'up', delay = 0, lazy = true, ...props }, ref) => {
    const animationMap = {
      up: 'slide-up',
      down: 'slide-down',
      left: 'slide-left',
...
export const ScaleIn = React.forwardRef<HTMLDivElement, ScaleInProps>(
  ({ children, delay = 0, lazy = true, ...props }, ref) => (
    <AnimatedElement ref={ref} animation="scale-in" delay={delay} lazy={lazy} {...props}>
      {children}
    </AnimatedElement>
  )
)
