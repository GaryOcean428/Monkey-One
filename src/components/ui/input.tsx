        ...
        <div className="relative">
          ...
          {label && (
            <label
              htmlFor={inputId}
              className={cn(
                'ease-gentle pointer-events-none absolute left-3 transition-all duration-200',
                'peer-placeholder-shown:text-muted-foreground peer-placeholder-shown:top-3 peer-placeholder-shown:text-base',
                'peer-focus:text-primary peer-focus:top-1 peer-focus:text-xs',
                (isFocused || hasValue) && 'text-primary top-1 text-xs'
              )}
            >
              {label}
            </label>
          ...
          {error && (
            <p className="text-destructive animate-slide-in-bottom mt-1 text-sm">{error}</p>
          )}
          {helperText && !error && (
    ...
    return (
      <div className="space-y-1">
        ...
        <input
          type={type}
          id={inputId}
          className={cn(inputVariants({ variant, state: error ? 'error' : state }), className)}
          ref={ref}
          {...props}
        />
        {error && <p className="text-destructive animate-slide-in-bottom text-sm">{error}</p>}
