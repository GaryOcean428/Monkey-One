  ...
  "group pointer-events-auto relative flex w-full items-center justify-between space-x-4 overflow-hidden rounded-lg border p-4 pr-8 shadow-lg transition-all data-[swipe=cancel]:translate-x-0 data-[swipe=end]:translate-x-[var(--radix-toast-swipe-end-x)] data-[swipe=move]:translate-x-[var(--radix-toast-swipe-move-x)] data-[swipe=move]:transition-none data-[state=open]:animate-in data-[state=closed]:animate-out data-[swipe=end]:animate-out data-[state=closed]:fade-out-80 data-[state=closed]:slide-out-to-right-full data-[state=open]:slide-in-from-top-full data-[state=open]:sm:slide-in-from-bottom-full backdrop-blur-sm",
  {
    variants: {
      variant: {
        ...
        success: "border-success bg-success/10 text-success-foreground backdrop-blur-sm border-success/20",
        warning: "border-warning bg-warning/10 text-warning-foreground backdrop-blur-sm border-warning/20", 
        info: "border-info bg-info/10 text-info-foreground backdrop-blur-sm border-info/20",
        glass: "border-white/20 bg-white/10 text-foreground backdrop-blur-md shadow-[0_8px_32px_0_rgba(31,38,135,0.37)]",
      ...
      animation: {
        ...
        slide: "data-[state=open]:animate-slide-in-right data-[state=closed]:animate-slide-out-right",
      }
    },
    defaultVariants: {
      variant: "default",
...
>(({ className, variant, size, animation, icon, showIcon = true, children, ...props }, ref) => {
  ...
  return (
    ...
    >
      ...
      {variant === 'glass' && (
        <div className="absolute inset-0 rounded-lg bg-gradient-to-r from-white/5 to-white/10 pointer-events-none" />
      )}
      
      {variant === 'accent' && (
        <div className="absolute inset-0 rounded-lg bg-gradient-to-r from-accent-500/5 to-accent-600/10 pointer-events-none" />
      )}
    </ToastPrimitives.Root>
  )
})
...
>(({ className, variant = 'default', ...props }, ref) => (
  <ToastPrimitives.Action
    ref={ref}
    className={cn(
      "inline-flex h-8 shrink-0 items-center justify-center rounded-md px-3 text-sm font-medium transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 hover-scale",
      {
        'bg-background/80 border border-border hover:bg-background text-foreground': variant === 'default',
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Close
    ref={ref}
    className={cn(
      "absolute right-2 top-2 rounded-md p-1 text-foreground/50 opacity-0 transition-all duration-200 hover:text-foreground hover:bg-accent/50 focus:opacity-100 focus:outline-none focus:ring-2 group-hover:opacity-100 hover:scale-110 active:scale-95",
...
>(({ className, value = 0, max = 100, variant = 'default', ...props }, ref) => {
  ...
  return (
    <div
      ref={ref}
      className={cn(
        "w-full bg-muted/30 rounded-full h-1 mt-3 overflow-hidden",
        className
      )}
      {...props}
    >
...
const ToastWithUndo = React.forwardRef<
  React.ElementRef<typeof Toast>,
  ToastWithUndoProps
>(({ title, description, onUndo, undoLabel = 'Undo', variant = 'default', duration = 5000 }, ref) => {
      ...
      <div className="flex items-center space-x-3 flex-1">
        ...
        <div className="flex-1">
          ...
          {typeof progress === 'number' && (
            <ToastProgress value={progress} variant={variant} />
          )}
        </div>
      </div>
    </Toast>
