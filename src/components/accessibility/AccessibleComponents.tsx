...
export function FocusIndicator({ children, className, variant = 'default' }: FocusIndicatorProps) {
  const focusClasses = {
...
export function AccessibleField({
  ...
  return (
    <div className={cn('accessible-field space-y-2', className)}>
      <label htmlFor={fieldId} className="block text-sm leading-6 font-medium">
        {label}
        {required && (
      ...
      <div className="relative">
        {React.cloneElement(children as React.ReactElement, {
          ...
          'aria-describedby': [helpText ? helpId : null, hasError ? errorId : null]
            .filter(Boolean)
            .join(' '),
      ...
      {hasError && (
        <p {...getErrorProps(fieldName)} className="text-destructive text-sm">
          {errors[fieldName]}
        </p>
      )}
    </div>
    ...
    >
      {items.map((item, index) => (
        <div
          ...
          className={cn(
            'list-item cursor-pointer rounded p-2',
            'focus:ring-ring focus:ring-2 focus:outline-none',
            index === selectedIndex && 'bg-primary text-primary-foreground',
            index === focusedIndex && 'ring-ring ring-2'
          )}
