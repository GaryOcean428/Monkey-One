...
const ThoughtItem: React.FC<ThoughtItemProps> = ({ thought }) => {
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0, y: -20 }}
      className={`mb-3 rounded-lg border p-3 ${colorClass}`}
    >
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center justify-between gap-x-2">
            <span className="truncate text-sm font-medium capitalize">
              {thought.type}
              {thought.agentId && (
                <span className="ml-2 text-xs opacity-70">Agent: {thought.agentId}</span>
              )}
            </span>
          ...
          {(thought.taskId || thought.collaborationId) && (
            <div className="mt-2 flex items-center gap-2 text-xs opacity-70">
              {thought.taskId && <Badge variant="outline">Task: {thought.taskId}</Badge>}
          ...
          {(thought.importance !== undefined || thought.confidence !== undefined) && (
            <div className="mt-2 flex items-center gap-2 text-xs">
              {thought.importance !== undefined && (
                <Badge variant="secondary">Importance: {thought.importance.toFixed(2)}</Badge>
              )}
              {thought.confidence !== undefined && (
                <Badge variant="secondary">Confidence: {thought.confidence.toFixed(2)}</Badge>
              )}
            </div>
          )}
        </div>
...
export function ThoughtLoggerPanel() {
  const { thoughts, filters, stats, getFilteredThoughts, setFilters, clearFilters, clearThoughts } =
    useThoughtLogger()
  ...
  return (
    <BasePanel title="Thought Logger" className="flex h-full flex-col">
      <div className="mb-4 flex items-center gap-2">
        ...
        <Button variant="outline" size="icon" onClick={clearThoughts}>
          <X className="h-4 w-4" />
        </Button>
      </div>
        ...
        {showFilters && (
          ...
          >
            <div className="flex gap-2">
              ...
              <Select value={selectedTask} onValueChange={setSelectedTask} className="flex-1">
      ...
      <Tabs defaultValue="thoughts" className="flex-1">
        ...
        <TabsContent value="stats">
          <div className="grid grid-cols-2 gap-4">
            <Card className="p-4">
              <h3 className="mb-2 font-medium">Thoughts by Type</h3>
            ...
            <Card className="p-4">
              <h3 className="mb-2 font-medium">Thoughts by Agent</h3>
              <div className="space-y-1">
            ...
            <Card className="col-span-2 p-4">
              <h3 className="mb-2 font-medium">Thoughts by Task</h3>
              <div className="space-y-1">
