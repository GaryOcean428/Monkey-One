            ...
            <div className="absolute top-4 left-4 bg-white p-3 rounded-lg shadow-lg border max-w-xs">
              ...
              <div className="text-sm space-y-1">
                ...
                {hoveredNode.properties.name && (
                  <div><strong>Name:</strong> {hoveredNode.properties.name}</div>
                )}
                {hoveredNode.properties.key && (
                  <div><strong>Key:</strong> {hoveredNode.properties.key}</div>
                )}
                <div><strong>Created:</strong> {new Date(hoveredNode.metadata.createdAt).toLocaleDateString()}</div>
                <div><strong>Source:</strong> {hoveredNode.metadata.source || 'Unknown'}</div>
              </div>
            </div>
          )}
