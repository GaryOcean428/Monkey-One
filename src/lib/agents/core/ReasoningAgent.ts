...
export class ReasoningAgent extends BaseAgent {
  ...
  private formatPlan(plan: ReturnType<typeof this.createWorkflowPlan>): string {
    return `Workflow Plan (ID: ${plan.id})\n\n` +
      plan.steps.map((step, i) => 
        `${i + 1}. ${step.description} (${step.estimatedDuration}min)`
      ).join('\n');
  }
  ...
  private async decomposeTasks(analysis: ReturnType<typeof this.analyzeTask>): Promise<PlanningStep[]> {
