...
export class CortexAgent extends BaseAgent {
  ...
  private async storeExperience(experience: any) {
  ...
  private cleanupWorkingMemory() {
    ...
    if (items.length > maxWorkingMemoryItems) {
      const itemsToRemove = items
        .slice(0, items.length - maxWorkingMemoryItems)
        .map(([key]) => key);
  ...
  private findRelevantExperiences(content: string): any[] {
