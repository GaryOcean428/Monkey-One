...
export class ThalamusAgent extends BaseAgent {
  ...
  async processMessage(message: Message): Promise<Message> {
    ...
    try {
      ...
      return super.createResponse('Error processing message routing')
    }
  }
  ...
  private updateMetric(routeKey: string, latency: number, isError: boolean = false) {
    ...
    const newLatency =
      (currentMetrics.latency * currentMetrics.throughput + latency) / newThroughput
  ...
  private isEmotionalContent(message: Message): boolean {
    ...
    return emotionalKeywords.some(keyword => message.content.toLowerCase().includes(keyword))
  }
