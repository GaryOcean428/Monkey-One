...
interface RelationshipPattern {
  ...
  type: EdgeType;
  confidence: number;
  extractor: (match: RegExpMatchArray, entities: Map<string, EntityExtraction>) => {
...
export class MLEntityExtractor {
  ...
  private initializePatterns(): void {
    ...
    this.entityPatterns.push(
      {
        pattern: /(?:the\s+)?(\w+(?:-\w+)*)\s+(?:service|microservice|api|application|app|backend|frontend)/gi,
        type: 'Service',
        confidence: 0.9,
    ...
    this.entityPatterns.push(
      {
        ...
        validator: (match, context) => {
          const envVarIndicators = /env|environment|config|setting|variable|secret|key|token|url|host|port|password/i;
      ...
      {
        pattern: /(?:config|configuration|setting|env|environment)\s+(?:variable\s+)?(?:called\s+|named\s+)?([A-Z_][A-Z0-9_]*)/gi,
        type: 'Configuration',
        confidence: 0.95
      }
    );
    ...
    this.entityPatterns.push(
      {
        pattern: /(?:(\w+)\s+)?(?:postgres|postgresql|mysql|mongodb|redis|elasticsearch|cassandra|dynamodb|sqlite)\s*(?:database|db|cluster|instance)?/gi,
        type: 'Database',
        confidence: 0.9,
        contextBoost: (text, match) => {
    ...
    this.entityPatterns.push(
      {
        pattern: /(?:in|on|to)\s+(production|staging|development|dev|prod|test|qa)\s+(?:environment|env)/gi,
        type: 'Environment',
        confidence: 0.95
      },
      {
  ...
  private initializeRelationshipPatterns(): void {
    this.relationshipPatterns.push(
      {
        pattern: /(\w+(?:-\w+)*)\s+(?:requires?|needs?)\s+([A-Z_][A-Z0-9_]*|[\w\s-]+?)(?:\s+(?:to|for|in)|[.,]|$)/gi,
        type: 'REQUIRES',
        confidence: 0.9,
        extractor: (match, entities) => this.findEntityPair(match[1], match[2], entities)
      },
      {
        pattern: /(\w+(?:-\w+)*)\s+(?:depends?\s+on|relies\s+on)\s+([\w\s-]+?)(?:\s+(?:to|for|in)|[.,]|$)/gi,
        type: 'DEPENDS_ON',
        confidence: 0.95,
        extractor: (match, entities) => this.findEntityPair(match[1], match[2], entities)
      },
      {
        pattern: /(\w+(?:-\w+)*)\s+(?:connects?\s+to|talks\s+to|communicates\s+with)\s+([\w\s-]+?)(?:\s+(?:via|through|using)|[.,]|$)/gi,
        type: 'CONNECTS_TO',
        confidence: 0.85,
        extractor: (match, entities) => this.findEntityPair(match[1], match[2], entities)
      },
      {
        pattern: /(\w+(?:-\w+)*)\s+(?:is\s+)?deployed\s+(?:to|on|in)\s+([\w\s-]+?)(?:\s+(?:environment|cluster)|[.,]|$)/gi,
        type: 'DEPLOYED_TO',
        confidence: 0.9,
        extractor: (match, entities) => this.findEntityPair(match[1], match[2], entities)
      },
      {
        pattern: /(\w+(?:-\w+)*)\s+(?:was\s+)?(?:caused\s+by|due\s+to|because\s+of)\s+([\w\s-]+?)(?:[.,]|$)/gi,
        type: 'CAUSED_BY',
        confidence: 0.85,
        extractor: (match, entities) => this.findEntityPair(match[1], match[2], entities)
      },
  ...
  private findBestMatch(
    ...
    // Partial match
    const partial = candidates.find(([key, entity]) => 
      key.includes(target) || 
      target.includes(key) ||
    ...
    for (const sentence of sentences) {
      ...
      if (entitiesInSentence.length > 1) {
        entitiesInSentence.forEach(entity => {
          entity.properties.confidence = Math.min(1.0, 
            (entity.properties.confidence || 0.5) + 0.05
          );
        });
      }
    }
