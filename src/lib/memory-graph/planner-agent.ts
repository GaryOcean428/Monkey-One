...
export class PlannerAgent {
  ...
  async analyzeRisks(serviceId: string): Promise<RiskAnalysis> {
    ...
    const serviceDeps = this.graph
      .query({ depth: 1 })
      .edges.filter(
        edge => edge.from === serviceId && ['DEPENDS_ON', 'REQUIRES'].includes(edge.type)
      )
  ...
  analyzeDependencies(entityId: string): DependencyAnalysis {
    ...
    const directDeps = this.graph
      .query({
        depth: 1,
      })
      .edges.filter(
        edge => edge.from === entityId && ['DEPENDS_ON', 'REQUIRES'].includes(edge.type)
      )
      .map(edge => edge.to)
    ...
    const findTransitive = (nodeId: string, depth: number) => {
      ...
      const deps = this.graph
        .query({ depth: 1 })
        .edges.filter(
          edge => edge.from === nodeId && ['DEPENDS_ON', 'REQUIRES'].includes(edge.type)
        )
        .map(edge => edge.to)
    ...
    const dependents = this.graph
      .query({
        depth: 1,
      })
      .edges.filter(edge => edge.to === entityId && ['DEPENDS_ON', 'REQUIRES'].includes(edge.type))
      .map(edge => edge.from)
  ...
  analyzeImpact(entityId: string): ImpactAnalysis {
    ...
    const directImpact = this.graph.getNeighbors(entityId, 'in').map(node => node.id)
  ...
  private findRequiredConfigurations(serviceId: string): string[] {
    ...
    const requirements = this.graph
      .query({ depth: 1 })
      .edges.filter(edge => edge.from === serviceId && edge.type === 'REQUIRES')
      .map(edge => edge.to)
  ...
  private findExistingConfigurations(serviceId: string): string[] {
    ...
    const existing = this.graph
      .query({ depth: 1 })
      .edges.filter(edge => edge.from === serviceId && edge.type === 'PROVIDES')
      .map(edge => edge.to)
  ...
  private findCriticalPath(entityId: string): string[] {
    ...
    const dfs = (nodeId: string, currentPath: string[]) => {
      ...
      const deps = this.graph
        .query({ depth: 1 })
        .edges.filter(edge => edge.from === nodeId && edge.type === 'DEPENDS_ON')
        .map(edge => edge.to)
