...
import type { Node, Edge, NodeType, EdgeType, GraphQuery, GraphQueryResult } from './types'
...
export class MemoryGraph {
  ...
  query(params: GraphQuery): GraphQueryResult {
    ...
    if (params.nodeType) {
      ...
      resultNodes = Array.from(this.nodes.values()).filter(node =>
    ...
    const resultEdges = Array.from(this.edges.values()).filter(edge => {
  ...
  private expandNodes(nodeIds: string[], depth: number): Set<string> {
    ...
    const queue: Array<{ id: string; level: number }> = nodeIds.map(id => ({ id, level: 0 }))
  ...
  getStats(): {
    ...
    const totalDegree = Array.from(this.nodes.keys()).reduce(
      (sum, nodeId) => sum + this.getCentralityScore(nodeId),
      0
    )
