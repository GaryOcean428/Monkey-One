...
export class AgentIntegrationHub {
  ...
  private messageHandlers: Map<AgentMessageType, (message: AgentMessage) => Promise<AgentMessage | null>> = new Map();
  ...
  async queryKnowledge(query: KnowledgeQuery): Promise<KnowledgeResponse> {
    ...
    try {
      ...
      if (query.maxResults) {
        ...
        if (results.relationships) results.relationships = results.relationships.slice(0, query.maxResults);
        if (results.insights) results.insights = results.insights.slice(0, query.maxResults);
      }

      const processingTime = Date.now() - startTime;
      const totalResults = (results.entities?.length || 0) + 
  ...
  private applyEntityFilters(entities: Node[], filters: NonNullable<KnowledgeQuery['filters']>): Node[] {
    ...
    if (filters.confidence) {
      filtered = filtered.filter(entity => 
        (entity.metadata.confidence || 1.0) >= filters.confidence!
      );
    }
  ...
  async integrateWithExecutionAgent(agentId: string): Promise<void> {
    ...
    const actionableRecs = recommendations.filter(rec => 
      rec.actions.length > 0 && rec.effort.level !== 'high'
    );
