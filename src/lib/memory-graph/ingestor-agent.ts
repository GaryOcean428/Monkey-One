...
export class IngestorAgent {
  ...
  async ingestStructuredData(data: {
  ...
  private extractEntities(text: string): EntityExtraction[] {
    ...
    const servicePattern = /(?:(\w+(?:-\w+)*)\s+(?:service|api|backend|frontend|microservice)|(?:service|api)\s+(\w+(?:-\w+)*))/gi;
    let match;
    ...
    while ((match = envVarPattern.exec(text)) !== null) {
      ...
      if (match[1].length >= 3 && (match[1].includes('_') || /^(API|DB|URL|KEY|SECRET|TOKEN|HOST|PORT|PASSWORD)/.test(match[1]))) {
        entities.push({
          id: `env:${match[1]}`,
    ...
    // Database detection
    const dbPattern = /(?:(\w+)\s+(?:database|db)|(?:postgres|mysql|mongodb|redis|postgresql)\s*(?:database|db)?)/gi;
    ...
    const envPattern = /(?:(?:to|in|on)\s+)?(production|staging|development|dev|prod|test)\s+environment/gi;
  ...
  private extractRelationships(
    ...
    const findEntity = (name: string): EntityExtraction | undefined => {
      ...
      return entityLookup.get(cleanName) || 
             entityLookup.get(cleanName.replace(/\s+/g, '')) ||
             entityLookup.get(cleanName.replace(/-/g, ''));
    };
    
    // Pattern: "X requires Y" - handle env vars specially
    const requiresPattern = /(\w+(?:-\w+)*)\s+requires?\s+([A-Z_][A-Z0-9_]*|[\w\s-]+?)(?:\s+(?:and|configuration|variable|env)|\.|$)/gi;
    let match;
    ...
    const dependsPattern = /(\w+(?:-\w+)*)\s+depends?\s+on\s+(\w+(?:\s+\w+)*?)(?:\s+(?:and|database|service)|\.|$)/gi;
    ...
    const connectsPattern = /(\w+(?:-\w+)*)\s+connects?\s+to\s+(\w+(?:\s+\w+)*?)(?:\s+(?:and|database|service|cache)|\.|$)/gi;
    ...
    const deployedPattern = /(\w+(?:-\w+)*)\s+(?:deployed\s+to|is\s+deployed\s+to)\s+(\w+(?:\s+\w+)*?)(?:\s+(?:and|environment)|\.|$)/gi;
    ...
    const causedByPattern = /(\w+(?:-\w+)*)\s+(?:was\s+)?(?:caused\s+by|due\s+to)\s+(\w+(?:\s+\w+)*?)(?:\s+(?:and|failure|issue)|\.|$)/gi;
    ...
    // Pattern: "X manages Y"
    const managesPattern = /(\w+(?:-\w+)*)\s+manages?\s+(\w+(?:\s+\w+)*?)(?:\s+(?:and|database|service)|\.|$)/gi;
    ...
    // Pattern: "X owns Y"
    const ownsPattern = /(\w+(?:-\w+)*)\s+owns?\s+(\w+(?:\s+\w+)*?)(?:\s+(?:and|service|api)|\.|$)/gi;
  ...
  extractEntitiesWithContext(text: string, context?: {
    domain?: string;
    source?: string;
    timestamp?: Date;
  }): EntityExtraction[] {
  ...
  async ingestMultipleTexts(
    ...
    for (let i = 0; i < texts.length; i += batchSize) {
      ...
      await Promise.all(
        batch.map(({ content, source, metadata }) =>
          this.ingestText(content, source)
        )
      );
    }
  }
