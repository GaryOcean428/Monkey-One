...
export class PerformanceMonitor {
  ...
  recordOperation(
    type: keyof PerformanceMetrics['operations'],
    duration: number,
  ...
  getPerformanceAnalysis(): {
    ...
    const trends = {
      ...
      capacity:
        this.metrics.system.memoryUsage.percentage > 70 ? ('high' as const) : ('medium' as const),
    }
  ...
  private updateErrorMetrics(): void {
    ...
    // Count recent errors
    const recentErrors = this.errorLog.filter(error => error.timestamp.getTime() > oneHourAgo)
    ...
    recentErrors.forEach(error => {
      this.metrics.errors.byType[error.type] = (this.metrics.errors.byType[error.type] || 0) + 1
    })
  }
  ...
  private runHealthChecks(): void {
    ...
    const memoryCheck: HealthCheck = {
      name: 'Memory Usage',
      status:
        memUsage > this.thresholds.memoryUsage.critical
          ? 'critical'
          : memUsage > this.thresholds.memoryUsage.warning
            ? 'warning'
            : 'healthy',
    ...
    const performanceCheck: HealthCheck = {
      name: 'Performance',
      status:
        avgQueryTime > this.thresholds.queryTime.critical
          ? 'critical'
          : avgQueryTime > this.thresholds.queryTime.warning
            ? 'warning'
            : 'healthy',
...
export function timed(
  ...
  operationType: keyof PerformanceMetrics['operations']
) {
