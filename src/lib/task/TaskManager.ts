...
export class TaskManager extends EventEmitter {
  ...
  async createTask(definition: Omit<TaskDefinition, 'id' | 'status' | 'createdAt' | 'updatedAt'>): Promise<string> {
  ...
  private async startContinuousTask(task: TaskDefinition, inputs?: Record<string, any>): Promise<void> {
  ...
  private async executeTask(task: TaskDefinition, inputs?: Record<string, any>): Promise<void> {
    try {
      // Load required tools
      const toolInstances = await Promise.all(
        task.tools.map(toolId => this.loadTool(toolId))
      );
