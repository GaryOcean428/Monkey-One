...
export abstract class BaseAgent implements Agent {
   ...
   */
  public async handleDelegation(request: DelegationRequest): Promise<boolean> {
   ...
   */
  protected validateTaskRequirements(step: WorkflowStep): void {
    const requiredCapabilities = step.metadata.requiredCapabilities || []
    const missingCapabilities = requiredCapabilities.filter(
      cap => !this.capabilities.some(agentCap => agentCap.id === cap)
    )
   ...
   */
  protected estimateTaskComplexity(step: WorkflowStep, context: Record<string, any>): number {
   ...
   * Log task progress
   */
  protected logProgress(step: WorkflowStep, progress: number, message: string): void {
    console.log(`[Agent ${this.id}] Task ${step.id}: ${progress}% - ${message}`)
  }

  /**
   * Handle task error
   */
  protected async handleTaskError(step: WorkflowStep, error: Error): Promise<void> {
    console.error(`[Agent ${this.id}] Error in task ${step.id}:`, error)
    ...
    if (handoffCriteria) {
      ...
      if (targetAgent) {
        const handoffSuccess = await handoffManager.executeHandoff(this, targetAgent, {
          ...step.context,
          error: error.message,
