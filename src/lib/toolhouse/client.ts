...
export class ToolhouseClient {
  ...
  // Pinecone Tools
  async pineconeUpsert(vectors: number[][], metadata: Record<string, any>[], config: PineconeConfig): Promise<void> {
  ...
  async pineconeSearch(vector: number[], config: PineconeConfig, topK: number = 10): Promise<any[]> {
