...
export class RouterCalibrator {
  ...
  public calibrateThreshold(targetStrongPct: number = 0.5): CalibrationResults {
    ...
    while (left <= right) {
      ...
      const strongPct =
        (results.distribution.high + results.distribution.superior) / this.sampleQueries.length
...
if (require.main === module) {
  ...
  calibrator
    .saveResults(results, path.join(__dirname, '../data/calibration_results.json'))
