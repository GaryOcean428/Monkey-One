...
class MemoryManager {
  ...
  private cleanup(): void {
    ...
    // Remove old items
    const validItems = items.filter(([, item]) => (item.timestamp || 0) > cutoff)
  ...
  private updateMetrics(): void {
    ...
    this.metrics = {
      ...
      storageUsage: items.reduce(
        (size, item) => size + new TextEncoder().encode(JSON.stringify(item)).length,
        0
      ),
    }
  }
  ...
  async search(
    ...
    results = results.filter(item => item.content.toLowerCase().includes(query.toLowerCase()))
  ...
  getRecent(count: number, type?: string): MemoryItem[] {
    ...
    return items.sort((a, b) => (b.timestamp || 0) - (a.timestamp || 0)).slice(0, count)
  }
