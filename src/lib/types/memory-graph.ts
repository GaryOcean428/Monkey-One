...
export type NodeType = 
  // AI & Chat Context
  | 'Message' | 'Conversation' | 'Context' | 'Intent' | 'Entity'
  | 'Agent' | 'Model' | 'Prompt' | 'Response' | 'Tool'
  
  // Knowledge & Memory
  | 'Concept' | 'Fact' | 'Rule' | 'Pattern' | 'Memory'
  | 'Document' | 'Code' | 'Function' | 'Variable'
  
  // User & Session
  | 'User' | 'Session' | 'Preference' | 'Goal' | 'Task'
  | 'Workflow' | 'Step' | 'Decision' | 'Outcome'
  
  // System & Infrastructure
  | 'Service' | 'API' | 'Database' | 'Configuration'
  | 'Error' | 'Event' | 'Metric' | 'Log';
...
export type EdgeType =
  ...
  | 'RESPONDS_TO' | 'FOLLOWS' | 'REFERENCES' | 'MENTIONS'
  | 'CLARIFIES' | 'CONTRADICTS' | 'SUPPORTS' | 'EXTENDS'
  
  // Semantic Relationships
  | 'IS_A' | 'HAS_A' | 'PART_OF' | 'SIMILAR_TO'
  | 'CAUSES' | 'ENABLES' | 'PREVENTS' | 'REQUIRES'
  
  // Temporal Relationships
  | 'BEFORE' | 'AFTER' | 'DURING' | 'TRIGGERS'
  | 'LEADS_TO' | 'RESULTS_IN' | 'DEPENDS_ON'
  
  // Agent Relationships
  | 'CREATED_BY' | 'USED_BY' | 'PROCESSED_BY' | 'HANDLED_BY'
  | 'ASSIGNED_TO' | 'DELEGATED_TO' | 'COLLABORATED_ON'
  
  // System Relationships
  | 'CONNECTS_TO' | 'CALLS' | 'RETURNS' | 'STORES'
  | 'LOGS' | 'MONITORS' | 'CONFIGURES' | 'DEPLOYS';
...
export interface GraphEvent {
  ...
  type: 'nodeAdded' | 'nodeUpdated' | 'nodeDeleted' | 
        'edgeAdded' | 'edgeUpdated' | 'edgeDeleted' |
        'queryExecuted' | 'memoryPruned' | 'snapshotCreated';
  timestamp: Date;
  data: any;
  source: string;
}
