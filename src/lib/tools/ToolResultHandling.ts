...
export class ToolResultHandling {
  static withResultValidation<T>(tool: Tool, handler: ResultHandler<T>): Tool {
    return {
      name: tool.name,
  ...
  static withAsyncTransformation<T>(tool: Tool, transform: (result: unknown) => Promise<T>): Tool {
    return {
      name: tool.name,
      description: tool.description,
      execute: async (args: Record<string, unknown>) => {
  ...
  static withErrorMapping(tool: Tool, errorMap: Record<string, string | RegExp>): Tool {
    return {
      name: tool.name,
      description: tool.description,
      execute: async (args: Record<string, unknown>) => {
