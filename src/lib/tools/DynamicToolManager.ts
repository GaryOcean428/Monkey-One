...
export class DynamicToolManager extends EventEmitter {
  ...
  async registerTool(tool: Omit<DynamicTool, 'id' | 'performance' | 'lastUsed' | 'usageCount'>): Promise<string> {
  ...
  private async validateInput(tool: DynamicTool, input: any): Promise<{ isValid: boolean; errors: string[] }> {
    const validationPrompt = `
  ...
  private async updateToolMetrics(data: { tool: DynamicTool; result: ToolExecutionResult }): Promise<void> {
    const { tool, result } = data;
    const updatedTool = { ...tool };

    // Update performance metrics
    const totalExecutions = tool.usageCount + 1;
    updatedTool.performance.latency = (tool.performance.latency * tool.usageCount + result.duration) / totalExecutions;
    updatedTool.performance.successRate = (tool.performance.successRate * tool.usageCount + (result.success ? 1 : 0)) / totalExecutions;
    updatedTool.performance.errorRate = (tool.performance.errorRate * tool.usageCount + (result.success ? 0 : 1)) / totalExecutions;
